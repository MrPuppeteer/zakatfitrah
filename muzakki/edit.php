...
if (isset($_POST['update'])) {
  ...
  if (empty($nama) || $jumlah_tanggungan === "") {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Kolom nama dan jumlah tanggungan wajib diisi.';
  ...
  $check = mysqli_query($mysqli, "SELECT * FROM bayarzakat WHERE nama_KK='$nama'");
  $ch = mysqli_fetch_assoc($check);

  if (!empty($ch['nama_KK'])) {
    if ($jumlah_tanggungan < $ch['jumlah_tanggunganyangdibayar']) {
      $_SESSION['flash_status'] = 'Error';
      $_SESSION['flash_message'] = 'Jumlah tanggungan tidak boleh lebih kecil dari jumlah tanggungan yang dibayar pada data Bayar Zakat.';
      header("location: edit.php?id=$id");
      exit();
    } else {
      $update = "UPDATE bayarzakat SET jumlah_tanggungan='$jumlah_tanggungan' WHERE nama_KK='$nama'";
      $up = mysqli_query($mysqli, $update);
    }
  }

  $sql = "UPDATE muzakki SET nama_muzakki='$nama', jumlah_tanggungan='$jumlah_tanggungan', keterangan='$keterangan' WHERE id_muzakki='$id'";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data muzakki berhasil diperbarui.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data muzakki gagal diperbarui.';
    header('location: index.php');
    exit();
  }
}
...
$result = mysqli_query($mysqli, "SELECT * FROM muzakki WHERE id_muzakki='$id'");

while ($res = mysqli_fetch_array($result)) {
  $nama = $res['nama_muzakki'];
...
<head>
  ...
  <title>Memperbarui Data Muzakki</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Memperbarui Data Muzakki</h2>
          <form name="muzakki_update" action="" method="POST">
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div class="sm:col-span-2">
                <label for="nama" class="block mb-2 text-sm font-medium text-white">Nama Muzakki</label>
                <input type="text" name="nama" id="nama" value="<?php echo $nama ?>" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500" placeholder="Masukan nama" required="">
              </div>
              <div class="w-full">
                <label for="jumlah_tanggungan" class="block mb-2 text-sm font-medium text-white">Jumlah Tanggungan</label>
                <input type="number" name="jumlah_tanggungan" id="jumlah_tanggungan" value="<?php echo $jumlah_tanggungan ?>" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500" placeholder="Masukan jumlah tanggungan" required="">
              </div>
              <div class="sm:col-span-2">
                <label for="keterangan" class="block mb-2 text-sm font-medium text-white">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="8" class="block p-2.5 w-full text-sm rounded-lg border bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500" placeholder="Masukan keterangan di sini"><?php echo $keterangan ?></textarea>
              </div>
            </div>
