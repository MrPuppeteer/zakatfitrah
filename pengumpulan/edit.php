...
if (isset($_POST['update'])) {
  ...
  $check = mysqli_query($mysqli, "SELECT * FROM muzakki WHERE id_muzakki='$id_muzakki'")
    or die("Could not execute the select query.");
  $row = mysqli_fetch_assoc($check);

  $nama = $row['nama_muzakki'];
  $jumlah_tanggungan = $row['jumlah_tanggungan'];

  if (empty($nama) || $jumlah_tanggungan === "" || empty($jenis_bayar) || $jumlah_tanggunganyangdibayar === "") {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Kolom nama, jumlah tanggungan, jenis bayar, jumlah tanggungan yang dibayar wajib diisi. Salah satu atau beberapa kolom kosong.';
    header("location: edit.php?id=$id");
    exit();
  }

  if ($jenis_bayar === "beras") {
    $bayar_beras = $jumlah_tanggunganyangdibayar * 2.5;
  } else if ($jenis_bayar === "uang") {
    $bayar_uang = $jumlah_tanggunganyangdibayar * 45000;
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Jenis pembayaran tidak valid!';
    header("location: edit.php?id=$id");
    exit();
  }

  if (empty($bayar_beras) && empty($bayar_uang)) {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Salah satu atribut bayar beras atau bayar uang harus diisi.';
    header("location: edit.php?id=$id");
    exit();
  }

  if ($jumlah_tanggungan < 0 || $jumlah_tanggunganyangdibayar < 0) {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Jumlah tanggungan tidak boleh negatif.';
    header("location: edit.php?id=$id");
    exit();
  }

  if ($jumlah_tanggungan < $jumlah_tanggunganyangdibayar) {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Jumlah tanggungan yang dibayar tidak boleh melebihi jumlah tanggungan.';
    header("location: edit.php?id=$id");
    exit();
  }
  ...
  if (empty($row)) {
    $_SESSION['flash_status'] = "Error";
    $_SESSION['flash_message'] = "Nama Kepala Keluarga tidak ditemukan di Data Muzakki.";
    header("location: edit.php?id=$id");
    exit();
  }

  if ($row['jumlah_tanggungan'] !== $jumlah_tanggungan) {
    $_SESSION['flash_status'] = "Error";
    $_SESSION['flash_message'] = "Jumlah tanggungan tidak sesuai dengan Data Muzakki.";
    header("location: edit.php?id=$id");
    exit();
  }

  $sql = "UPDATE bayarzakat SET nama_KK='$nama', jumlah_tanggungan='$jumlah_tanggungan', jenis_bayar='$jenis_bayar', jumlah_tanggunganyangdibayar='$jumlah_tanggunganyangdibayar', bayar_beras='$bayar_beras', bayar_uang='$bayar_uang' WHERE id_zakat='$id'";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data bayar zakat berhasil diperbarui.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data bayar zakat gagal diperbarui.';
    header('location: index.php');
    exit();
  }
}
...
$sql = "SELECT * FROM bayarzakat WHERE id_zakat='$id'";
$result = mysqli_query($mysqli, $sql);

while ($res = mysqli_fetch_array($result)) {
  $nama = $res['nama_KK'];
  $jumlah_tanggungan = $res['jumlah_tanggungan'];
  $jenis_bayar = $res['jenis_bayar'];
  $jumlah_tanggunganyangdibayar = $res['jumlah_tanggunganyangdibayar'];
}

if (empty($nama)) {
  $_SESSION['flash_status'] = 'Error';
  $_SESSION['flash_message'] = 'Id data tidak ditemukan.';
  header('location: index.php');
  exit();
}

$muzakki = mysqli_query($mysqli, "SELECT * FROM muzakki");
if (mysqli_num_rows($muzakki) === 0) {
  $_SESSION['flash_status'] = 'Error';
  $_SESSION['flash_message'] = 'Mohon masukan data muzakki terlebih dahulu.';
  header('location: index.php');
  exit();
}
?>
...
<head>
  ...
  <title>Memperbarui Data Bayar Zakat</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Memperbarui Data Bayar Zakat</h2>
          <form name="bayarzakat_update" method="POST" action="">
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div class="sm:col-span-2">
                <label for="id_muzakki" class="block mb-2 text-sm font-medium text-white">Nama Muzakki</label>
                <select id="id_muzakki" name="id_muzakki" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <?php while ($muz = mysqli_fetch_array($muzakki)) { ?>
                    <option value="<?php echo $muz['id_muzakki']; ?>" <?php if ($muz['nama_muzakki'] === $nama) echo "selected"; ?>><?php echo $muz['nama_muzakki']." (".$muz['jumlah_tanggungan']." Tanggungan) "; ?></option>
                  <?php } ?>
                </select>
              </div>
              <div class="w-full">
                <label for="jumlah_tanggunganyangdibayar" class="block mb-2 text-sm font-medium text-white">Jumlah Tanggungan Yang Dibayar</label>
                <input type="number" name="jumlah_tanggunganyangdibayar" id="jumlah_tanggunganyangdibayar" value="<?php echo $jumlah_tanggunganyangdibayar ?>" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500" placeholder="Masukan jumlah tanggungan yang dibayar" required="">
              </div>
              <div class="w-full">
                <label for="jenis_bayar" class="block mb-2 text-sm font-medium text-white">Jenis Pembayaran</label>
                <select id="jenis_bayar" name="jenis_bayar" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <option <?php if ($jenis_bayar === "beras") echo "selected"; ?> value="beras">Beras - 2.5 kg per orang</option>
                  <option <?php if ($jenis_bayar === "uang") echo "selected"; ?> value="uang">Uang - Rp. 45.000 per orang</option>
                </select>
              </div>
            </div>
            <input type="hidden" name="id" value="<?php echo $id ?>">
            <button name="update" value="update" type="submit" class="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-green-700 rounded-lg focus:ring-4 focus:ring-green-900 hover:bg-green-800">
              Perbarui data
            </button>
          </form>
        </div>
