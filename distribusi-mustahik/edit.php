...
if (isset($_POST['update'])) {
  ...
  $sql = "UPDATE mustahik_lainnya SET nama='$nama', kategori='$kategori', hak='$hak' WHERE id_mustahiklainnya='$id'";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data mustahik lainnya berhasil diperbarui.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data mustahik lainnya gagal diperbarui.';
    header('location: index.php');
    exit();
  }
}
...
$result = mysqli_query($mysqli, "SELECT * FROM mustahik_lainnya WHERE id_mustahiklainnya='$id'");
...
<head>
  ...
  <title>Memperbarui Data Mustahik Lainnya</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Memperbarui Data Mustahik Lainnya</h2>
            ...
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              ...
              <div class="w-full">
                ...
                <select id="kategori" name="kategori" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <option value="amilin" <?php if ($kategori === "amilin") echo "selected" ?>>Amilin</option>
                  <option value="fisabilillah" <?php if ($kategori === "fisabilillah") echo "selected" ?>>Fisabilillah</option>
                  <option value="mualaf" <?php if ($kategori === "mualaf") echo "selected" ?>>Mualaf</option>
                  <option value="ibnu sabil" <?php if ($kategori === "ibnu sabil") echo "selected" ?>>Ibnu Sabil</option>
                </select>
              </div>
            </div>
