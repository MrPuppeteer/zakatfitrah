...
if (isset($_POST['submit'])) {
  ...
  $sql = "INSERT INTO mustahik_warga (nama, kategori, hak) VALUES ('$nama', '$kategori', '$hak')";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data mustahik warga berhasil ditambahkan.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data mustahik warga gagal ditambahkan.';
    header('location: index.php');
    exit();
  }
}
...
<head>
  ...
  <title>Menambahkan Data Mustahik Warga</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Menambahkan Data Mustahik Warga Baru</h2>
              ...
              <div class="sm:col-span-2">
                ...
                <select id="id_muzakki" name="id_muzakki" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <?php while ($muz = mysqli_fetch_array($muzakki)) { ?>
                    <option value="<?php echo $muz['id_muzakki']; ?>"><?php echo $muz['nama_muzakki']; ?></option>
                  <?php } ?>
                </select>
              </div>
              <div class="w-full">
