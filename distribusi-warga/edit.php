...
if (isset($_POST['update'])) {
  ...
  $check1 = mysqli_query($mysqli, "SELECT * FROM muzakki WHERE id_muzakki='$id_muzakki'")
    or die("Could not execute the select query.");
  $row1 = mysqli_fetch_assoc($check1);

  $check2 = mysqli_query($mysqli, "SELECT * FROM kategori_mustahik WHERE nama_kategori='$kategori'")
    or die("Could not execute the select query.");
  $row2 = mysqli_fetch_assoc($check2);

  $nama = $row1['nama_muzakki'];
  $hak = $row2['jumlah_hak'];

  if (empty($id_muzakki) || empty($kategori)) {
  ...
  $sql = "UPDATE mustahik_warga SET nama='$nama', kategori='$kategori', hak='$hak' WHERE id_mustahikwarga='$id'";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data mustahik warga berhasil diperbarui.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data mustahik warga gagal diperbarui.';
    header('location: index.php');
    exit();
  }
}
...
$result = mysqli_query($mysqli, "SELECT * FROM mustahik_warga WHERE id_mustahikwarga='$id'");
...
<head>
  ...
  <title>Memperbarui Data Mustahik Warga</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Memperbarui Data Mustahik Warga</h2>
          <form name="bayarzakat_add" action="" method="POST">
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div class="sm:col-span-2">
                <label for="id_muzakki" class="block mb-2 text-sm font-medium text-white">Nama</label>
                <select id="id_muzakki" name="id_muzakki" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <?php while ($muz = mysqli_fetch_array($muzakki)) { ?>
                    <option value="<?php echo $muz['id_muzakki']; ?>" <?php if ($muz['nama_muzakki'] === $nama) echo "selected"; ?>><?php echo $muz['nama_muzakki']; ?></option>
                  <?php } ?>
                </select>
              </div>
              <div class="w-full">
                ...
                <select id="kategori" name="kategori" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500">
                  <option value="fakir" <?php if ($kategori === "fakir") echo "selected"; ?>>Fakir</option>
                  <option value="miskin" <?php if ($kategori === "miskin") echo "selected"; ?>>Miskin</option>
                  <option value="mampu" <?php if ($kategori === "mampu") echo "selected"; ?>>Mampu</option>
                </select>
              </div>
            </div>
