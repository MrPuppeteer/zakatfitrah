...
if (isset($_POST['update'])) {
  ...
  if (empty($nama) || empty($jumlah_hak)) {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Kolom nama dan jumlah hak tidak boleh kosong.';
    header("location: edit.php?id='$id'");
    exit();
  }

  $sql = "UPDATE kategori_mustahik SET nama_kategori='$nama', jumlah_hak='$jumlah_hak' WHERE id_kategori='$id'";
  $result = mysqli_query($mysqli, $sql);

  if ($result) {
    $_SESSION['flash_status'] = 'Success';
    $_SESSION['flash_message'] = 'Data kategori mustahik berhasil diperbarui.';
    header('location: index.php');
    exit();
  } else {
    $_SESSION['flash_status'] = 'Error';
    $_SESSION['flash_message'] = 'Data kategori mustahik gagal diperbarui.';
    header('location: index.php');
    exit();
  }
}
...
$result = mysqli_query($mysqli, "SELECT * FROM kategori_mustahik WHERE id_kategori='$id'");

while ($res = mysqli_fetch_array($result)) {
  $nama = $res['nama_kategori'];
  $jumlah_hak = $res['jumlah_hak'];
}
...
<head>
  ...
  <title>Memperbarui Data Kategori Mustahiks</title>
    ...
    <main class="mx-auto p-4">
      <section class="bg-gray-900 relative shadow-md sm:rounded-lg">
        <div class="py-8 px-4 mx-auto max-w-2xl lg:py-16">
          <h2 class="mb-4 text-xl font-bold text-white">Memperbarui Data Kategori Mustahik</h2>
          <form name="kategori_mustahik_update" action="" method="POST">
            <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
              <div class="sm:col-span-2">
                <label for="nama" class="block mb-2 text-sm font-medium text-white">Nama Kategori</label>
              ...
              <div class="w-full">
                <label for="jumlah_hak" class="block mb-2 text-sm font-medium text-white">Jumlah Hak</label>
                <input step="0.01" type="number" name="jumlah_hak" id="jumlah_hak" value="<?php echo $jumlah_hak ?>" class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-green-500 focus:border-green-500" placeholder="Masukan jumlah hak" required="">
              </div>
            </div>
